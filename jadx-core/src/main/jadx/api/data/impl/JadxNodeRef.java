...
public class JadxNodeRef implements IJavaNodeRef {
	...
	private static final Comparator<IJavaNodeRef> COMPARATOR = new Comparator<IJavaNodeRef>() {
		@Override
		public int compare(IJavaNodeRef node1, IJavaNodeRef node2) {
			int typeComparison = node1.getType().compareTo(node2.getType());
			if (typeComparison != 0) {
				return typeComparison;
			}
			
			int declaringClassComparison = node1.getDeclaringClass().compareTo(node2.getDeclaringClass());
			if (declaringClassComparison != 0) {
				return declaringClassComparison;
			}
			
			return node1.getShortId().compareTo(node2.getShortId());
		}
	};
