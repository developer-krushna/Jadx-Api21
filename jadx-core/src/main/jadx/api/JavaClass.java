...
public final class JavaClass implements JavaNode {
		...
		if (methodsCount != 0) {
			...
			for (MethodNode m : cls.getMethods()) {
				if (!m.contains(AFlag.DONT_GENERATE)) {
					JavaMethod javaMethod = rootDecompiler.convertMethodNode(m);
					if (javaMethod != null) {
						mths.add(javaMethod);
					}
				}
	...
	*/
	...
	public List<JavaClass> getDependencies() {
		...
		List<JavaClass> dependencies = new ArrayList<>();
		for (ClassNode dependencyNode : cls.getDependencies()) {
			JavaClass javaClass = d.convertClassNode(dependencyNode);
			if (javaClass != null) {
