...
public class SSAVar {
    ...
    Iterator<RegisterArg> iterator = useList.iterator();
    while (iterator.hasNext()) {
        RegisterArg registerArg = iterator.next();
        if (registerArg == arg) {
            iterator.remove();
        }
    }
    ...
    if (usedInPhi != null) {
        Iterator<PhiInsn> iterator = usedInPhi.iterator();
        while (iterator.hasNext()) {
            PhiInsn insn = iterator.next();
            if (insn == phiInsn) {
                iterator.remove();
            }
        }
