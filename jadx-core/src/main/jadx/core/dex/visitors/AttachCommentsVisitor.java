...
private void updateCommentsData(@Nullable ICodeData data) {
    if (data == null) {
        ...
        this.clsCommentsMap = new HashMap<>();
        List<ICodeComment> comments = data.getComments();
        for (ICodeComment comment : comments) {
            String declaringClass = comment.getNodeRef().getDeclaringClass();
            if (!clsCommentsMap.containsKey(declaringClass)) {
                clsCommentsMap.put(declaringClass, new ArrayList<>());
            }
            clsCommentsMap.get(declaringClass).add(comment);
        }
    }
}
}
