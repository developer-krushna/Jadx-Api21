...
public class SignatureProcessor extends AbstractVisitor {
	...
	*/
	interface ParentClassVisitor {
		void visit(ClassNode parent);
	}
	
	interface TypeVisitor {
		void visit(ArgType argType);
	}
	...
	List<ArgType> generics, ArgType superClass, List<ArgType> interfaces) {
		...
		addGenericType(typeParams, superClass);
		for (ArgType interfaceType : interfaces) {
			addGenericType(typeParams, interfaceType);
		}
		...
		if (cls.isInner()) {
			...
			List<ArgType> parentGenerics = new ArrayList<>();
			ClassNode parent = cls;
			while (parent != null) {
				parentGenerics.addAll(parent.getGenericTypeParameters());
				parent = parent.getParentClass();
			}
			knownTypeParams.addAll(parentGenerics);
		} else {
		...
		for (String typeParam : typeParams) {
			fixedGenerics.add(ArgType.genericType(typeParam));
		}
