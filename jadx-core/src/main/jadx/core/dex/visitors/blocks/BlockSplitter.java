...
public class BlockSplitter extends AbstractVisitor {
	...
	public void visit(MethodNode mth) {
		...
		List<BlockNode> blocksToRemove = new ArrayList<>();
		for (BlockNode block : mth.getBasicBlocks()) {
			if (removeEmptyBlock(block)) {
				blocksToRemove.add(block);
			}
		}
		mth.getBasicBlocks().removeAll(blocksToRemove);
	...
	*/
	...
	private static void removeInsns(MethodNode mth) {
		for (BlockNode block : mth.getBasicBlocks()) {
			Iterator<InsnNode> iterator = block.getInstructions().iterator();
