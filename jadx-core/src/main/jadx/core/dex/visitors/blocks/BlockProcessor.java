...
public class BlockProcessor extends AbstractVisitor {
    ...
    if (block.contains(AFlag.LOOP_START) || block.contains(AFlag.LOOP_END)) {
        ...
        if (predsCount > 1) {
            ...
            if (BlockUtils.checkFirstInsn(block, new Predicate<InsnNode>() {
                @Override
                public boolean test(InsnNode insn) {
                    return insn.contains(AType.EXC_HANDLER);
                }
            })) {
                return false;
            }
    ...
    if (preds.size() > 2) {
        ...
        for (BlockNode block : blocks) {
            BlockSplitter.replaceConnection(block, loopHeader, preHeader);
        }
        return true;
    }
    return false;
    ...
    Iterator<BlockNode> iterator = mth.getBasicBlocks().iterator();
    while (iterator.hasNext()) {
        BlockNode block = iterator.next();
