...
public class FixMultiEntryLoops {
    ...
    List<SpecialEdgeAttr> multiEntryLoops = new ArrayList<>();
    for (SpecialEdgeAttr edge : specialEdges) {
        if (edge.getType() == SpecialEdgeType.BACK_EDGE && !isSingleEntryLoop(edge)) {
            multiEntryLoops.add(edge);
        }
    }
    ...
    try {
        List<SpecialEdgeAttr> crossEdges = new ArrayList<>();
        for (SpecialEdgeAttr edge : specialEdges) {
            if (edge.getType() == SpecialEdgeType.CROSS_EDGE) {
                crossEdges.add(edge);
            }
        }
