...
public class ShadowFieldVisitor extends AbstractVisitor {
    ...
    // Remove fields with unique names
    Iterator<Map.Entry<String, List<FieldNode>>> iterator = mapByName.entrySet().iterator();
    while (iterator.hasNext()) {
        List<FieldNode> fields = iterator.next().getValue();
        if (fields.size() == 1) {
            iterator.remove();
        }
    }
    ...
    for (List<FieldNode> fields : mapByName.values()) {
        ...
        if (fromThisCls && fields.size() == 2) {
            // only one super class contains the same field => can use super
            ...
            // several super classes contain the same field => can't use super, need cast to exact class
    ...
    Map<String, List<FieldNode>> groupByName = new HashMap<>();
    for (FieldNode field : allFields) {
        String fieldName = field.getName();
        List<FieldNode> fieldsWithSameName = groupByName.get(fieldName);
        if (fieldsWithSameName == null) {
            fieldsWithSameName = new ArrayList<>();
            groupByName.put(fieldName, fieldsWithSameName);
        }
        fieldsWithSameName.add(field);
    }
    return groupByName;
}
