...
public final class TypeUpdate {
	...
	*/
	...
	private Map<InsnType, ITypeListener> initListenerRegistry() {
		...
		registry.put(InsnType.CONST, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return sameFirstArgListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.MOVE, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return moveListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.PHI, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
		...
		registry.put(InsnType.AGET, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return arrayGetListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.APUT, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return arrayPutListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.IF, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return ifListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.ARITH, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return arithListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.NEG, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
		...
		registry.put(InsnType.NOT, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return suggestAllSameListener(updateInfo, insn, arg, candidateType);
			}
		...
		registry.put(InsnType.CHECK_CAST, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return checkCastListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.INVOKE, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return invokeListener(updateInfo, insn, arg, candidateType);
			}
		});
		
		registry.put(InsnType.CONSTRUCTOR, new ITypeListener() {
			@Override
			public TypeUpdateResult update(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
				return invokeListener(updateInfo, insn, arg, candidateType);
			}
	...
	private TypeUpdateResult invokeListener(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
		...
		if (invoke.getInstanceArg() == arg) {
			...
			if (typeVarsMap.isEmpty()) {
				...
				ArgType finalReturnType = returnType;
				ArgType[] finalArgTypes = new ArgType[argsCount];
				for (int i = 0; i < argsCount; i++) {
					finalArgTypes[i] = argTypes.get(i);
				}
				return applyInvokeTypes(updateInfo, invoke, argsCount, knownTypeVars, new Supplier<ArgType>() {
					@Override
					public ArgType get() {
						return finalReturnType;
					}
				}, new Function<Integer, ArgType>() {
					@Override
					public ArgType apply(Integer argNum) {
						return finalArgTypes[argNum];
					}
				});
			}
			// resolve types before apply
			ArgType[] resolvedArgTypes = new ArgType[argsCount];
			for (int i = 0; i < argsCount; i++) {
				resolvedArgTypes[i] = typeUtils.replaceClassGenerics(candidateType, argTypes.get(i));
			}
			final ArgType[] finalResolvedArgTypes = resolvedArgTypes;
			final ArgType finalReturnType = typeUtils.replaceTypeVariablesUsingMap(returnType, typeVarsMap);
			return applyInvokeTypes(updateInfo, invoke, argsCount, knownTypeVars, new Supplier<ArgType>() {
				@Override
				public ArgType get() {
					return finalReturnType;
				...
				@Override
				public ArgType apply(Integer argNum) {
					return finalResolvedArgTypes[argNum];
				}
			});
		}
		return SAME;
