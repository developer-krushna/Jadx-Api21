...
public class TypeCompare {
	...
	public TypeCompare(RootNode root) {
		...
		this.reversedComparator = getReversedComparator(comparator);
	}
	
	/* Traditional Method instead of  Lambda Expression*/
	/* By @developer-krushna*/
	private Comparator<ArgType> getReversedComparator(final Comparator<ArgType> comparator) {
		return new Comparator<ArgType>() {
			@Override
			public int compare(ArgType arg1, ArgType arg2) {
				// Reversing the natural order of ArgType objects
				return comparator.compare(arg2, arg1);
			}
		};
	}
