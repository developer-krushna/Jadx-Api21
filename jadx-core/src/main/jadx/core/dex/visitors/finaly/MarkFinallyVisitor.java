...
public class MarkFinallyVisitor extends AbstractVisitor {
	...
	public void visit(MethodNode mth) {
		...
		try {
			...
			if (applied) {
				...
				Iterator<TryCatchBlockAttr> iterator = clearedTryBlocks.iterator();
				
				while (iterator.hasNext()) {
					TryCatchBlockAttr tb = iterator.next();
					if (tb.isMerged() || tb.getHandlers().isEmpty()) {
						iterator.remove();
					}
				}
	...
	*/
	...
	private static List<BlockNode> getPathStarts(MethodNode mth, BlockNode bottom, BlockNode bottomFinallyBlock) {
		List<BlockNode> pathStarts = new ArrayList<>();
		for (BlockNode pred : bottom.getPredecessors()) {
			if (pred != bottomFinallyBlock) {
				pathStarts.add(pred);
			}
		}
		if (bottom == mth.getExitBlock()) {
			List<BlockNode> additionalBlocks = new ArrayList<>();
			for (BlockNode r : pathStarts) {
				for (BlockNode predecessor : r.getPredecessors()) {
					if (predecessor != bottomFinallyBlock) {
						additionalBlocks.add(predecessor);
					}
				}
			}
			pathStarts.addAll(additionalBlocks);
		}
		return pathStarts;
	}
