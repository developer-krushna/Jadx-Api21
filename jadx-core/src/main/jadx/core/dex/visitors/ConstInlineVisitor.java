...
public class ConstInlineVisitor extends AbstractVisitor {
	...
	*/
	...
	private static boolean replaceConst(MethodNode mth, InsnNode constInsn, InsnArg constArg) {
		...
		boolean allUsesCanIgnore = true;
		for (InsnArg arg : ssaVar.getUseList()) {
			if (!ConstInlineVisitor.canIgnoreInsn(arg)) {
				allUsesCanIgnore = false;
				break;
			}
		}
	...
	private static boolean canIgnoreInsn(InsnArg arg) {
		if (arg instanceof RegisterArg) {
			return canIgnoreInsn((RegisterArg) arg);
		}
		return false; // Handle other types of InsnArgs if needed
	}
