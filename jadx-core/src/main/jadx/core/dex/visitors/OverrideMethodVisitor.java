...
public class OverrideMethodVisitor extends AbstractVisitor {
	...
	Set<IMethodDetails> baseMethods, @Nullable MethodOverrideAttr attr) {
		...
		if (attr == null) {
			...
			List<IMethodDetails> cleanOverrideList = removeDuplicates(overrideList);
			return applyOverrideAttr(mth, cleanOverrideList, baseMethods, false);
		}
		// trace stopped at already processed method -> start merging
		List<IMethodDetails> mergedOverrideList = mergeLists(overrideList, attr.getOverrideList());
		List<IMethodDetails> cleanOverrideList = removeDuplicates(mergedOverrideList);
		Set<IMethodDetails> mergedBaseMethods = mergeSets(baseMethods, attr.getBaseMethods());
		return applyOverrideAttr(mth, cleanOverrideList, mergedBaseMethods, true);
	}
	
	private List<IMethodDetails> removeDuplicates(List<IMethodDetails> list) {
		List<IMethodDetails> result = new ArrayList<>();
		Set<IMethodDetails> set = new HashSet<>();
		for (IMethodDetails item : list) {
			if (set.add(item)) {
				result.add(item);
			}
		}
	...
	private List<IMethodDetails> mergeLists(List<IMethodDetails> list1, List<IMethodDetails> list2) {
		List<IMethodDetails> mergedList = new ArrayList<>(list1);
		mergedList.addAll(list2);
		return mergedList;
	}
	
	private Set<IMethodDetails> mergeSets(Set<IMethodDetails> set1, Set<IMethodDetails> set2) {
		Set<IMethodDetails> mergedSet = new HashSet<>(set1);
	...
	Set<IMethodDetails> baseMethods, boolean update) {
		...
		boolean dontRename = false;
		for (IMethodDetails methodDetails : overrideList) {
