...
public class DebugInfoAttachVisitor extends AbstractVisitor {
        ...
        try {
            ...
            if (insn != null) {
                ...
                if (insn.getType() != InsnType.NOP) {
                    Integer count = linesStat.get(line);
                    if (count == null) {
                        count = 0;
                    }
                    linesStat.put(line, count + 1);
                }
            }
        } catch (Exception e) {
    ...
    List<Map.Entry<Integer, Integer>> repeatingLines = new ArrayList<>();
    for (Map.Entry<Integer, Integer> entry : linesStat.entrySet()) {
        if (entry.getValue() > 3) {
            repeatingLines.add(entry);
        }
    }
