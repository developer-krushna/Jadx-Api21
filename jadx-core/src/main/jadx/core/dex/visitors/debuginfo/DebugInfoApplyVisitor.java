...
public class DebugInfoApplyVisitor extends AbstractVisitor {
	...
	private static void searchDebugInfoByOffset(MethodNode mth, SSAVar ssaVar) {
		...
		for (RegisterArg arg : ssaVar.getUseList()) {
			int offset = getInsnOffsetByArg(arg, mth);
			maxOffset = Math.max(maxOffset, offset);
		}
		if (maxOffset == -1) {
			return;
		}
		int startOffset = getInsnOffsetByArg(ssaVar.getAssign(), mth);
		...
		for (ILocalVar localVar : debugInfoAttr.getLocalVars()) {
			if (localVar.getRegNum() == regNum) {
				...
				if (isInside(startOffset, startAddr, endAddr) || isInside(maxOffset, startAddr, endAddr)) {
					if (Consts.DEBUG_TYPE_INFERENCE) {
	...
	private static int getInsnOffsetByArg(RegisterArg arg, MethodNode mth) {
		InsnNode insn = arg.getParentInsn();
		if (insn != null) {
			return insn.getOffset();
		}
		// Handle the case when the instruction is not found (return a default value or handle it based on your logic)
		return -1;
	}
		...
		for (SSAVar ssaVar : ssaVars) {
			for (PhiInsn phiInsn : ssaVar.getUsedInPhi()) {
				...
				if (names.size() == 1) {
					...
					setNameForInsn(phiInsn, iterator.next());
				}
			}
		}
	}
