...
public class AttachTryCatchVisitor extends AbstractVisitor {
    ...
    for (ClassInfo classInfo : catchTypes) {
        ArgType currentType = classInfo.getType();
        if (maxType == null || comparator.compare(currentType, maxType) > 0) {
            maxType = currentType;
        }
    }
    ...
    if (maxType != null) {
        ...
        throw new JadxRuntimeException("Failed to get max type from catch list: " + catchTypes);
    }
}
