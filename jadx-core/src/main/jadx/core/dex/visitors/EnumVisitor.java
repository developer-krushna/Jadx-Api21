...
private boolean searchValuesField(EnumData data) {
    ...
    List<FieldNode> valuesCandidates = new ArrayList<>();
    for (FieldNode field : data.cls.getFields()) {
        if (field.getAccessFlags().isStatic() && field.getType().isArray() &&
                Objects.equals(field.getType().getArrayRootElement(), clsType)) {
            valuesCandidates.add(field);
        }
    }
    ...
    if (valuesCandidates.size() > 1) {
        Iterator<FieldNode> iterator = valuesCandidates.iterator();
        while (iterator.hasNext()) {
            FieldNode field = iterator.next();
            if (!field.getAccessFlags().isSynthetic()) {
                iterator.remove();
            }
        }
    }
    ...
    if (valuesCandidates.size() > 1) {
        Optional<FieldNode> valuesOpt = Optional.empty();
        for (FieldNode field : valuesCandidates) {
            if (field.getName().equals("$VALUES")) {
                valuesOpt = Optional.of(field);
                break;
            }
        }
    ...
    if (valuesCandidates.size() != 1) {
        ...
        for (FieldNode field : valuesCandidates) {
            candidatesList.append(field).append(", ");
        }
        data.cls.addWarnComment("Found several \"values\" enum fields: " + candidatesList);
        return false;
    }
