...
public class ProcessAnonymous extends AbstractVisitor {
	...
	private void visitClassAndInners(ClassNode cls) {
		...
		for (ClassNode innerClass : innerClasses) {
			visitClassAndInners(innerClass);
		}
	}
	...
	private void mergeAnonymousDeps(RootNode root) {
		...
		for (ClassNode anonymousCls : root.getClasses()) {
			...
			if (attr != null) {
				...
				if (!useMap.containsKey(anonymousCls)) {
					useMap.put(anonymousCls, Collections.emptyList());
				}
				if (!inlineMap.containsKey(anonymousCls)) {
		...
		for (Map.Entry<ClassNode, List<ClassNode>> entry : useMap.entrySet()) {
			ClassNode key = entry.getKey();
			List<ClassNode> list = entry.getValue();
			if (list.isEmpty()) {
				added.clear();
	...
	private void updateDeps(ClassNode leafCls, Map<ClassNode, ClassNode> inlineMap, Set<ClassNode> added) {
		...
		while (true) {
			if (!added.add(current)) {
				Set<ClassNode> undoSet = new HashSet<>(added);
				for (ClassNode node : undoSet) {
					ProcessAnonymous.undoAnonymousMark(node);
				}
