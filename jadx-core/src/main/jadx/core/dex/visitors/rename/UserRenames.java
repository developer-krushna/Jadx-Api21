...
public class UserRenames {
	...
	public static void applyForNodes(RootNode root) {
		...
		List<ICodeRename> renames = codeData.getRenames();
		Map<String, List<ICodeRename>> groupedRenames = new HashMap<>();
		...
		for (ICodeRename rename : renames) {
			if (rename.getCodeRef() == null && rename.getNodeRef().getType() != IJavaNodeRef.RefType.PKG) {
				...
				if (!groupedRenames.containsKey(declaringClass)) {
					groupedRenames.put(declaringClass, new ArrayList<>());
				}
				groupedRenames.get(declaringClass).add(rename);
			}
		}
		
		for (Map.Entry<String, List<ICodeRename>> entry : groupedRenames.entrySet()) {
			String clsRawName = entry.getKey();
			List<ICodeRename> renamesForClass = entry.getValue();
			
			ClassInfo clsInfo = infoStorage.getCls(ArgType.object(clsRawName));
			if (clsInfo != null) {
				ClassNode cls = root.resolveClass(clsInfo);
				if (cls != null) {
					for (ICodeRename rename : renamesForClass) {
	...
	private static void applyPkgRenames(RootNode root, List<ICodeRename> renames) {
		...
		for (ICodeRename pkgRename : renames) {
