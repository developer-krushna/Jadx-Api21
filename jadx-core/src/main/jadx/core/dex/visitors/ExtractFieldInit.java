...
public class ExtractFieldInit extends AbstractVisitor {
	...
	private static void filterFieldsInit(List<FieldInitInfo> inits) {
		Set<FieldInfo> excludedFields = new HashSet<>();
		
		// Exclude fields initialized several times
		Map<FieldNode, Integer> fieldCounts = new HashMap<>();
		/* Traditional Method instead of  Lambda Expression*/
		/* By @developer-krushna*/
		for (FieldInitInfo fi : inits) {
			FieldNode fieldNode = fi.fieldNode;
			Integer count = fieldCounts.get(fieldNode);
			if (count == null) {
				fieldCounts.put(fieldNode, 1);
			} else {
				fieldCounts.put(fieldNode, count + 1);
			}
		}
		
		for (Map.Entry<FieldNode, Integer> entry : fieldCounts.entrySet()) {
			if (entry.getValue() > 1) {
				excludedFields.add(entry.getKey().getFieldInfo());
			}
		}
		...
		// Apply exclusion
		if (!excludedFields.isEmpty()) {
			Iterator<FieldInitInfo> iterator = inits.iterator();
			while (iterator.hasNext()) {
				FieldInitInfo fi = iterator.next();
				if (excludedFields.contains(fi.fieldNode.getFieldInfo())) {
					iterator.remove();
				}
			}
		}
	...
	private static List<FieldNode> processFieldsDependencies(ClassNode cls, List<FieldInitInfo> inits) {
		List<FieldNode> orderedFields = new ArrayList<>();
		for (FieldInitInfo initInfo : inits) {
			orderedFields.add(initInfo.fieldNode);
		}
		// collect dependent fields
		Map<FieldNode, List<FieldNode>> deps = new HashMap<>();
		for (FieldInitInfo initInfo : inits) {
			IndexInsnNode insn = initInfo.putInsn;
			boolean staticField = insn.getType() == InsnType.SPUT;
			InsnType useType = staticField ? InsnType.SGET : InsnType.IGET;
			insn.visitInsns(new Consumer<InsnNode>() {
				@Override
				public void accept(InsnNode subInsn) {
					if (subInsn.getType() == useType) {
						...
						if (fieldInfo.getDeclClass().equals(cls.getClassInfo())) {
							...
							if (depField != null) {
								List<FieldNode> depFields = deps.get(initInfo.fieldNode);
								if (depFields == null) {
									depFields = new ArrayList<>();
									deps.put(initInfo.fieldNode, depFields);
								}
		...
		// build new list with dependent fields before usage field
