	...
	*/
	...
	private static Map<ArgType, ArgType> mergeTypeMaps(Map<ArgType, ArgType> base, Map<ArgType, ArgType> addition) {
		...
		for (Map.Entry<ArgType, ArgType> entry : base.entrySet()) {
			...
			ArgType baseValue = entry.getValue();
			ArgType additionValue = addition.get(baseValue);
			if (additionValue != null) {
				map.put(key, additionValue);
			} else {
				map.put(key, baseValue);
			}
		}
		for (Map.Entry<ArgType, ArgType> entry : addition.entrySet()) {
			ArgType key = entry.getKey();
