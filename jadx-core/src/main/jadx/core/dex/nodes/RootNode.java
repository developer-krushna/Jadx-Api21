...
public class RootNode {
	...
	public void loadClasses(List<ILoadResult> loadedInputs) {
		for (ILoadResult loadedInput : loadedInputs) {
			loadedInput.visitClasses(new CustomClassConsumer());
		}
		...
		// Calculate statistics for loaded classes
		int mthCount = 0;
		int insnsCount = 0;
		for (ClassNode classNode : classes) {
			mthCount += classNode.getMethods().size();
			for (MethodNode methodNode : classNode.getMethods()) {
				insnsCount += methodNode.getInsnsCount();
			}
		}
		LOG.info("Loaded classes: {}, methods: {}, instructions: {}", classes.size(), mthCount, insnsCount);
		
		// Sort classes by name, expect top classes before inner
		Collections.sort(classes, new Comparator<ClassNode>() {
			@Override
	...
	class CustomClassConsumer implements com.custom.Consumer<IClassData> {
		@Override
		public void accept(IClassData cls) {
			try {
	...
	private static void markDuplicatedClasses(List<ClassNode> classes) {
		Map<ClassInfo, List<ClassNode>> groupedClasses = new HashMap<>();
		for (ClassNode classNode : classes) {
			ClassInfo classInfo = classNode.getClassInfo();
			if (groupedClasses.containsKey(classInfo)) {
				groupedClasses.get(classInfo).add(classNode);
			} else {
				List<ClassNode> nodeList = new ArrayList<>();
				nodeList.add(classNode);
				groupedClasses.put(classInfo, nodeList);
			}
		}
		
		for (Map.Entry<ClassInfo, List<ClassNode>> entry : groupedClasses.entrySet()) {
			List<ClassNode> duplicates = entry.getValue();
			if (duplicates.size() > 1) {
				List<String> sources = new ArrayList<>();
				for (ClassNode duplicate : duplicates) {
					sources.add(duplicate.getInputFileName());
				}
				
				Collections.sort(sources);
				
				LOG.warn("Found duplicated class: {}, count: {}. Only one will be loaded!\n  {}",
				entry.getKey(), duplicates.size(), String.join("\n  ", sources));
				for (ClassNode cls : duplicates) {
					String thisSource = cls.getInputFileName();
					StringBuilder otherSourceStr = new StringBuilder();
					
					for (String s : sources) {
						if (!s.equals(thisSource)) {
							otherSourceStr.append(s).append("\n  ");
						}
					}
					
					String sortedSources = otherSourceStr.toString().trim();
					Arrays.sort(sortedSources.split("\n"));
	...
	private void initInnerClasses() {
		...
		for (ClassNode cls : inner) {
			...
			if (parent == null) {
				Iterator<Map.Entry<ClassInfo, ClassNode>> iterator = clsMap.entrySet().iterator();
				while (iterator.hasNext()) {
					Map.Entry<ClassInfo, ClassNode> entry = iterator.next();
					if (entry.getKey().equals(clsInfo)) {
						iterator.remove();
						break;
					}
				}
	...
	*/
	...
	public void notifyCodeDataListeners() {
		...
		for (ICodeDataUpdateListener listener : codeDataUpdateListeners) {
			listener.updated(codeData);
		}
	}
