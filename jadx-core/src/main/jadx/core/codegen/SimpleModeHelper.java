...
public class SimpleModeHelper {
	...
	public List<BlockNode> prepareBlocks() {
		...
		Iterator<BlockNode> iterator = blocksList.iterator();
		while (iterator.hasNext()) {
			BlockNode block = iterator.next();
			if (block.equals(mth.getEnterBlock()) || block.equals(mth.getExitBlock())) {
				iterator.remove();
			}
		}
	...
	private List<BlockNode> getSortedBlocks() {
		final List<BlockNode> list = new ArrayList<>(mth.getBasicBlocks().size());
		/* Traditional Method instead of  Lambda Expression*/
		/* By @developer-krushna*/
		BlockUtils.dfsVisit(mth, new com.custom.Consumer<BlockNode>() {
			@Override
