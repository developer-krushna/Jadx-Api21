...
public class AndroidResourcesUtils {
	...
	*/
	...
	private static void addResourceFields(ClassNode resCls, ResourceStorage resStorage, boolean rClsExists) {
		...
		class ResClsInfoFunction implements Function<String, ResClsInfo> {
			@Override
			public ResClsInfo apply(String name) {
				return getClassForResType(resCls, rClsExists, name);
			}
		}
		
		class FieldNodeFunction implements Function<String, FieldNode> {
			private final ClassNode typeCls;
			private final ResourceEntry resource;
			
			FieldNodeFunction(ClassNode typeCls, ResourceEntry resource) {
				this.typeCls = typeCls;
				this.resource = resource;
			}
			
			@Override
			public FieldNode apply(String name) {
				FieldInfo rFieldInfo = FieldInfo.from(typeCls.root(), typeCls.getClassInfo(), name, ArgType.INT);
		...
		for (ResourceEntry resource : resStorage.getResources()) {
			...
			ResClsInfo typeClsInfo = innerClsMap.get(resTypeName);
			if (typeClsInfo == null) {
				typeClsInfo = new ResClsInfoFunction().apply(resTypeName);
				innerClsMap.put(resTypeName, typeClsInfo);
			}
			
			FieldNode fieldNode = typeClsInfo.getFieldsMap().get(resName);
			if (fieldNode == null) {
				fieldNode = new FieldNodeFunction(typeClsInfo.getTypeCls(), resource).apply(resName);
				typeClsInfo.getFieldsMap().put(resName, fieldNode);
			}
	...
	private static ResClsInfo getClassForResType(ClassNode resCls, boolean rClsExists, String typeName) {
		...
		if (existCls != null) {
			...
			for (FieldNode field : existCls.getFields()) {
				resClsInfo.getFieldsMap().put(field.getName(), field);
			}
			return resClsInfo;
		}
