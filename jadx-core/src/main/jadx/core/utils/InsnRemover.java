...
public class InsnRemover {
	...
	private static void removeSsaVar(MethodNode mth, SSAVar ssaVar) {
		...
		if (allPhis) {
			List<RegisterArg> useListCopy = new ArrayList<>(ssaVar.getUseList());
			for (RegisterArg arg : useListCopy) {
				InsnNode parentInsn = arg.getParentInsn();
				if (parentInsn != null && parentInsn.getType() == InsnType.PHI) {
		...
		if (Consts.DEBUG_WITH_ERRORS) {
			StringBuilder errorMsg = new StringBuilder("Can't remove SSA var, still in use, count: ")
			.append(useCount)
			.append(", list:").append(ICodeWriter.NL);
			for (RegisterArg arg : ssaVar.getUseList()) {
				errorMsg.append("  ").append(arg).append(" from ").append(arg.getParentInsn()).append(ICodeWriter.NL);
			}
			throw new JadxRuntimeException(errorMsg.toString());
		}
	}
