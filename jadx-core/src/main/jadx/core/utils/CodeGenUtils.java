...
public class CodeGenUtils {
    ...
    if (!errors.isEmpty()) {
        Set<JadxError> uniqueErrors = new HashSet<>(errors);
        List<JadxError> sortedErrors = new ArrayList<>(uniqueErrors);
        Collections.sort(sortedErrors);
        for (JadxError err : sortedErrors) {
    ...
    if (commentsAttr != null) {
        List<String> formattedComments = commentsAttr.formatAndFilter(node.getCommentsLevel());
